...
| `get_map`            | read  | ASCII map (whole grid or a window) — layers `overview`, `terrain` (`.` land, `~` river, `#` lake, `%` sea), `supply`, `density`, `power`, `transit`                                                         |
...
| `find_tiles`         | read  | Tiles by kind (`empty_land`, `river`, `lake_shore`, `coastal_sea`, `road`, `power_line`, `plant`, `zoned_empty`, `building`, `not_connected_building`, `undersupplied_building`, `bus_stop`), nearest-first |
